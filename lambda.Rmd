...
```
A) Show where the distribution is centered at and compare it to the theoretical center of the distribution: The center (mean) of the thoeretical distribution is 1/lambda which equals 1/0.2 0r  5 in this case.  The mean of my simulations is:
```{r}
mean(randMeans)
```
...
Since the theoretical variance of the sample distribution is sigma squared and we already know that sigma (the standard deviation) is the same as the mean, or 5, and n in this example is 40, then the theoretical  variance and standard deviation should be sigma^2/40 and sigma/sqrt(40):
```{r, echo=FALSE}
...
I think the best way to show normalcy is to plot the thoeretical quantiles against the quantiles in my sample, showing the theoretically perfect normal distribution as a line:
```{r fig.height=3, fig.width=3}
qqnorm(randMeans)
qqline(randMeans)
```

Though it is not a perfect fit, it does appear to be "approximately" normal, with a bit of skewing in both tails.  We would probably see it approach near perfect normalcy if we chose a much larger sample size (larger than 40).

D) Evaluate the coverage of the confidence interval for 1/lambda: X+/- 1.96* S/sqrt(n)
